...
CG_BEFORE_BUFFER
uint32_t demo_buf3[BUFFERSIZE3]={0};
...
#define BUFFERSIZE6 200
CG_BEFORE_BUFFER
uint8_t demo_buf6[BUFFERSIZE6]={0};
...
uint32_t demo_scheduler(int *error,demoContext_t *demoContext)
{
    ...
    */
    ...
    FIFO<uint32_t,FIFOSIZE2,1,0> fifo2(demo_buf3);
    FIFO<uint8_t,FIFOSIZE3,1,0> fifo3(demo_buf4);
    FIFO<uint8_t,FIFOSIZE4,1,0> fifo4(demo_buf5);
    FIFO<uint8_t,FIFOSIZE5,1,0> fifo5(demo_buf6);
    ...
    */
    ...
    Duplicate2<uint8_t,200,uint8_t,200,uint8_t,200> dup0(fifo3,fifo4,fifo5);
    SDSTimedRecorder<uint8_t,200,uint32_t,50> fakeRecorder(fifo5,fifo2,demoContext->recConn_temperatureSensor);
    SDSTimedSensor<uint8_t,200,uint32_t,50> fakeSensor(fifo3,fifo2,demoContext->sensorConn_temperatureSensor,simple_timed_drift_correction,drift_data);
    FormatTemperature<uint8_t,200,float,50> formatTemp(fifo4,fifo0);
